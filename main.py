...
bot = telebot.TeleBot("7288327128:AAEyW_wQfyGBHvAU8Ct_J-A-7Pv1zWwuqn4")  # Замените "YOUR_BOT_TOKEN" на токен вашего бота
...
def generate_signal():
    try:
        random_image = random.choice([
            ...
            "IMG/26.jpg", "IMG/27.jpg", "IMG/28.jpg", "IMG/29.jpg"
        ])
        ...
        return "Error generating signal. Please try again later.", "IMG/error.jpg"
...
def handle_language(call):
    try:
        ...
        bot.send_message(call.message.chat.id, "Error occurred. Please try again later.")
...
# Функция для отображения главного меню (турецкий)
def main_menu_tr(message):
    try:
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📱KAYIT OL", callback_data="registration"))
        markup.add(types.InlineKeyboardButton("📚TALİMATLAR", callback_data="instruction"))
        markup.add(types.InlineKeyboardButton("💣SİNYAL VER💣", callback_data="signal"))
        ...
        print(f"Error in main_menu_tr: {e}")
        bot.send_message(message.chat.id, "Error occurred. Please try again later.")
    ...
    if call.data == "registration":
        ...
        if user_id in users:
            ...
            try:
                ...
                # Обработка ошибки отправки фото
                bot.send_message(chat_id,
                                 "Извините, произошла ошибка при отправке сигнала." if current_language == "ru" else "Üzgünüm, sinyal gönderilirken bir hata oluştu.",
                                 reply_markup=types.InlineKeyboardMarkup(
                                     types.InlineKeyboardButton(
                                         "📱Главное меню" if current_language == "ru" else "📱Ana Menü",
                                         callback_data="main_menu")
                                 ))
        else:
            bot.delete_message(chat_id, call.message.message_id)
